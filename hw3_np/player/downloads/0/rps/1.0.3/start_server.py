...
def end_by_forfeit(leaver: str, reason: str = "disconnected"):
    """任一玩家離開/逾時 → 另一位直接獲勝"""
    global game_over, game_over_notified, pointer_name, loser_name
    ...
    remaining = None
    for n in names:
        if n != leaver and n in players:
            remaining = n
            break

    # 若真的找不到 remaining，就不硬送 result
    if not remaining:
        game_over = True
    ...
    print(f"[HB-Server] {leaver} {reason}, {remaining} wins by forfeit", flush=True)
...
def check_player_timeout():
    ...
    with lock:
        ...
        for nm in names:
            last_seen = player_last_seen.get(nm, now)
            if now - last_seen > TIMEOUT_SEC:
                print(f"[HB-Server] {nm} timeout", flush=True)
                end_by_forfeit(nm, "timeout")
                return True
...
def handle(conn, addr):
    ...
    try:
        ...
        while True:
            ...
            if kind == "quit":
                print(f"[HB-Server] {name} requested quit", flush=True)
                break
            # ========== 第一階段：猜拳決定指人者 ==========
            if kind == "hand":
        ...
        with lock:
            ...
            # ✅ 若此時還沒 game_over，代表是「中途離開」
            if existed and not game_over:
                if len(players) >= 1:
                    # 直接用共用棄權結束
                    end_by_forfeit(name, "disconnected")
                else:
                    os._exit(0)
