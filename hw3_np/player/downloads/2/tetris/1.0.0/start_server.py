...
async def game_loop(room: GameRoom):
    ...
    # ===== 勝負判定（優先：TopOut -> Lines -> Score -> Draw）=====
    p1 = room.engine["P1"]
    p2 = room.engine["P2"]
    ...
    if p1_top != p2_top:
        ...
        # 同時 top out → 比行數，行數再同 → 比分數，再同 → 平手
        if p1_lines != p2_lines:
            winner_role = "P1" if p1_lines > p2_lines else "P2"
            win_detail = f"more lines ({p1_lines} vs {p2_lines}) after simultaneous top out"
        elif p1_score != p2_score:
            winner_role = "P1" if p1_score > p2_score else "P2"
            win_detail = f"higher score ({p1_score} vs {p2_score}) after simultaneous top out"
        else:
            winner_role = None
            win_detail = "draw (same lines and score after simultaneous top out)"
    else:
        # 2) 無人 top out（多半是時間到）→ 比行數，再比分數 → 否則平手
        if p1_lines != p2_lines:
            winner_role = "P1" if p1_lines > p2_lines else "P2"
    ...
    reason = "topout" if (p1_top or p2_top) else "timeup"

    results = [
        {
            "role":"P1",
            "username": p1_conn.user_id if p1_conn else None,  # ⭐ 改名為 username
        ...
        {
            "role":"P2",
            "username": p2_conn.user_id if p2_conn else None,  # ⭐ 改名為 username
    ...
    # 踢出觀戰者
    print(f"[GameServer] Kicking out {len(room.spectators)} spectators...")
    ...
    # 回報給 lobby（可附上 winner 資訊；lobby 如未使用會忽略）
    try:
        print(f"[GameServer] Reporting to lobby...", flush=True)

        # ✅ 用可連線的 host（避免 0.0.0.0）
        ...
        if not lobby_host or not lobby_port:
            raise RuntimeError(f"invalid lobby target host={lobby_host} port={lobby_port}")

        # ⭐ 改用原始 socket，不用 framing（lobby 不支援長度前綴）
        ...
        print(f"[GameServer] ✓ Sent {len(msg)} bytes to lobby", flush=True)

        try:
            resp_data = sock.recv(4096).decode("utf-8").strip()
            if resp_data:
                resp = json.loads(resp_data)
                print(f"[GameServer] ✓ Lobby acknowledged: {resp}", flush=True)
        except socket.timeout:
            print(f"[GameServer] ⚠ Lobby response timeout (but message sent)", flush=True)
        except Exception as e:
            print(f"[GameServer] ⚠ Failed to parse response: {e}", flush=True)

        sock.close()

    except Exception as e:
        print(f"[GameServer] ✗ Failed to report: {type(e).__name__}: {e}", flush=True)
    print(f"[GameServer] ✓ Game cleanup complete", flush=True)

    # ⭐ 給 GUI 時間吃到 MATCH_END 後自行收尾
...
async def broadcast_game_end(room: GameRoom):
    ...
    if room.early_end_reason:
        ...
        if room.early_winner is None:
            ...
            if p1_score != p2_score:
                winner_role = "P1" if p1_score > p2_score else "P2"
                win_detail = f"higher score after both disconnected"
            else:
                win_detail = "draw (both disconnected)"
    
    else:
        # 正常結束判定（TopOut > Lines > Score > Draw）
        p1_top = bool(p1.topout)
        p2_top = bool(p2.topout)
        p1_lines = int(p1.lines)
        p2_lines = int(p2.lines)
        ...
        if p1_top != p2_top:
            ...
            if p1_lines != p2_lines:
                winner_role = "P1" if p1_lines > p2_lines else "P2"
                win_detail = f"more lines after simultaneous top out"
            elif p1_score != p2_score:
                winner_role = "P1" if p1_score > p2_score else "P2"
                win_detail = f"higher score after simultaneous top out"
            else:
